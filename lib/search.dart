...
class SearchBar<T> extends StatefulWidget {
  ...
  final NumberBox listHeight;

  final VoidCallback callback;

  final NumberBox cancel;
  ...
  SearchBar({
    ...
    this.header,
    this.placeHolder,
    this.icon = const Icon(Icons.search, color: Colors.orange,),
    this.hintText = "",
    this.hintStyle = const TextStyle(color: Color.fromRGBO(142, 142, 147, 1), fontSize: 14),
    this.iconActiveColor = Colors.black,
    this.textStyle = const TextStyle(color: Colors.black),
    this.cancellationText = const Text("Exit"),
    ...
    this.searchBarPadding = const EdgeInsets.all(5),
...
class _SearchBarState<T> extends State<SearchBar<T>> with TickerProviderStateMixin, _ControllerListener<T> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.cancel.value == 3) {widget.cancel.value=1;cancelNoRefresh();}
    return Column(
